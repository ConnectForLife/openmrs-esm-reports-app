...
function getEditorState(initialCron: string): EditorState {
  ...
  if (scheduleType == ST_ADVANCED) {
    ...
    return {
      scheduleType,
      date: moment().set('year', parseInt(openMRSCron.year)).set('month', parseInt(openMRSCron.month) - 1).set('date', parseInt(openMRSCron.day)).toDate(),
      selectedDayOfMonth: undefined,
      selectedDaysOfWeek: [],
    ...
    return {
      ...
      date: undefined,
      selectedDayOfMonth: undefined,
      selectedDaysOfWeek: openMRSCron.dayOfWeek.split(',').map(dayId => parseInt(dayId)).map(dayId => DAYS_OF_WEEK[dayId - 1]),
      time: { hours: parseInt(openMRSCron.hours), minutes: parseInt(openMRSCron.minutes) },
      cron: null
    };
...
const SimpleCronEditor: React.FC<SimpleCronEditorProps> = ({ initialCron, onChange }) => {
  ...
  const buildCron = () => {
    ...
    if (selectedScheduleType == ST_ADVANCED) {
      ...
      setCron(`0 ${selectedTime.minutes} ${selectedTime.hours} ${editorState.date.getDate()} ${editorState.date.getMonth() + 1} ? ${editorState.date.getFullYear()}`);
      ...
      setCron(`0 ${selectedTime.minutes} ${selectedTime.hours} ? * ${editorState.selectedDaysOfWeek.map(dayOfWeek => (dayOfWeek.value))}`);
    ...
    return (
      <div className={styles.cronEditorField}>
        ...
        >
          {
            SCHEDULE_TYPES
              .filter(scheduleType => (initialScheduleType != ST_ADVANCED && scheduleType != ST_ADVANCED) || initialScheduleType == ST_ADVANCED)
              .map(scheduleType => (
  ...
  return (
    <div className={styles.cronContainer}>
      ...
      {editorState.scheduleType != ST_ADVANCED && <div className={styles.cronEditorFieldSeparator}>
        <span>{t('at', 'at')}</span>
      </div>}
