...
const EditScheduledReportForm: React.FC<EditScheduledReportForm> = ({ reportDefinitionUuid, reportRequestUuid, closePanel }) => {
  const { t } = useTranslation();
  const isTablet = useLayoutType() === 'tablet';

  const reportDefinition = getReportDefinition(reportDefinitionUuid);
  const reportDesigns = getReportDesignsSWR(reportDefinitionUuid);
  const { reportRequest } = getReportRequest(reportRequestUuid);
  ...
  const handleSubmit = useCallback(
    ...
    [closePanel, renderModeUuid, reportParameters, schedule]
  );
      ...
      <Stack gap={8} className={styles.container}>
        ...
        <div className={styles.outputFormatDiv}>
          ...
          >
            ...
            {
              reportDesigns && reportDesigns.map(reportDesign => (
